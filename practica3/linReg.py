...
class LinRegClassifier:
    ...
    def __init__(self, xTrain:pd.DataFrame, yTrain:pd.DataFrame, numIter:int=1000, lRate:float=0.001) -> None:
        ...
        self.coefs = np.zeros(xTrain.shape[1] + 1) + 1
    ...
    def train(self):
        for i in range(5):
            # print(np.array(self.coefs[:-1]))
            coefVars = np.concatenate((np.apply_along_axis(self.dmse, 0, np.array([self.coefs[:-1]])), [self.dmse(1)]))
            print(coefVars)
            self.coefs = np.apply_along_axis(lambda x: x[0] - self.lRate * x[1], 0, np.stack((self.coefs, coefVars)))
            print(self.coefs)
